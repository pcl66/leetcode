...
function currying(fn) {
  ...
  return function carried(...args1) {
    if(args1.length >= length) {
      return fn.call(this, ...args1)
    } else {
      return function(...args2) {
        return carried.call(this, ...args1, ...args2)
      }
    }
  }
}
...
// 大数字整数相加
function add2(a, b) {
  let aRight = a.length - 1
  let bRight = b.length - 1
  let carry = 0
  let resultArr = []
  while(aRight >= 0 || bRight >=0 || carry !== 0) {
    const i = aRight >= 0 ? Number(a[aRight]) : 0
    const j = bRight >= 0 ? Number(b[bRight]) : 0
    const sum = i + j + carry
